...
export default function ContentModal({children,media,id}) {
  ...
  const fetchData = async()=>{
      const {data} = await axios.get(`https://api.themoviedb.org/3/${media==="movie"?"movie":"tv"}/${id}?api_key=${process.env.REACT_APP_API_KEY}`);
      setContent(data);
  }
    const fetchVideo = async()=>{
      const {data} = await axios.get(`https://api.themoviedb.org/3/${media==="movie"?"movie":"tv"}/${id}/videos?api_key=${process.env.REACT_APP_API_KEY}`);
      if(data.results[0]){
